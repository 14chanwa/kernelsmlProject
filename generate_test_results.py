...
def generate_submission_file(classifier, use_bow=True):
    if use_bow:
    ...
    # Generate labels
    Yte0 = np.array(np.sign(classifier.predict(Xte0, Xte0.shape[0])), dtype=int)
    Yte1 = np.array(np.sign(classifier.predict(Xte1, Xte1.shape[0])), dtype=int)
    Yte2 = np.array(np.sign(classifier.predict(Xte2, Xte2.shape[0])), dtype=int)
